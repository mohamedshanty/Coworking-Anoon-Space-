...
async function calcDayRevenue(date) {
    ...
    const activeCourses = await prisma_1.prisma.course.findMany({
    ...
    if (activeCourses.length > 0) {
        ...
        const trainees = await prisma_1.prisma.trainee.findMany({
            where: { courseId: { in: courseIds } },
            select: { amountPaid: true, courseId: true },
        });
        const paidByCourse = new Map();
        ...
        for (const course of activeCourses) {
            ...
            if (totalPaid > 0) {
                const durationDays = Math.max(1, Math.round((new Date(course.endDate).getTime() - new Date(course.startDate).getTime()) / day));
    ...
    const debts = await prisma_1.prisma.debt.findMany({
        where: {
            status: "collected",
...
class DashboardService {
    async getSummary() {
        ...
        const liveVisitorCount = await prisma_1.prisma.session.count({
            where: { checkOut: null },
        });
        // todayRevenue
        const todayRevenue = await calcDayRevenue(now);
        // todayVisitCount: sessions checked in today
        const todayVisitCount = await prisma_1.prisma.session.count({
            where: {
                checkIn: { gte: todayStart, lte: todayEnd },
            },
        });
        // activeCoursesToday: courses where today falls between startDate and endDate
        const activeCoursesToday = await prisma_1.prisma.course.count({
            where: {
    ...
    `;
        ...
        const unpaidDebts = await prisma_1.prisma.debt.findMany({
            where: { status: "unpaid" },
        ...
        const expiringSubs = await prisma_1.prisma.subscription.findMany({
            where: {
                status: "active",
        ...
        const followUpCount = await prisma_1.prisma.$queryRaw `
    ...
    `;
        const followUpNeedsCount = followUpCount[0]?.count || 0;
