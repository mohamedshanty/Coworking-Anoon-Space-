...
const exportQuerySchema = zod_1.z.object({
    ...
    format: zod_1.z.enum(["xlsx"]).optional(),
    type: zod_1.z.enum(["reports", "history"]).optional(),
});
class ReportsController {
    async getPreview(req, res, next) {
        try {
            ...
            const [sessions, sales, expenses, bookings, courses, activeSubscriptions, settings] = await Promise.all([
                ...
                prisma_1.prisma.settings.findFirst(),
            ]);
            const r = (v) => Math.round((v + Number.EPSILON) * 100) / 100;
            // Visits
            const visitCount = sessions.length;
            const hoursRevenue = r(sessions.filter((s) => s.paymentStatus === "paid").reduce((sum, s) => sum + Number(s.amount), 0));
            // Subscribers
    ...
    async exportReport(req, res, next) {
        try {
            ...
            const collectedDebts = await prisma_1.prisma.debt.findMany({
                where: {
                    status: "collected",
            ...
            res.setHeader("Content-Disposition", `attachment; filename=${exportType}_${parsed.from}_${parsed.to}.xlsx`);
